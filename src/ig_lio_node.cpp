...
public:
  IG_LIO_NODE() : Node("ig_lio_node") {
    ...
    this->get_parameter("lidar_type", lidar_type_string_);

    // Determine lidar type...
    if (lidar_type_string_ == "velodyne") {
      lidar_type_ = LidarType::VELODYNE;
    } else if (lidar_type_string_ == "ouster") {
      lidar_type_ = LidarType::OUSTER;
    } else if (lidar_type_string_ == "livox") {
    ...
    // Initialize LIO, preprocess, and other configurations using parameters...

    // Setup subscribers
    imu_sub_ = this->create_subscription<sensor_msgs::msg::Imu>(
        imu_topic_, 10, std::bind(&IG_LIO_NODE::ImuCallBack, this, _1));

    // Depending on lidar_type_, setup appropriate subscriber...
    // Setup publishers
    odom_pub_ = this->create_publisher<nav_msgs::msg::Odometry>("/lio_odom", 10);
    // Setup other publishers...
...
void SigHandle(int sig) {
  ...
  // Declare other callback methods and any helper functions...
  ...
  std::string lidar_type_string_;
  LidarType lidar_type_; // You might need to define this enum based on your application needs
...
int main(int argc, char **argv) {
  ...
  auto node = std::make_shared<IG_LIO_NODE>();
  rclcpp::spin(node);
  rclcpp::shutdown();
  return 0;
}
